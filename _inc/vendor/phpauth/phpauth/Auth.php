...
class Auth
{
    ...
    */
    ...
    public function register($username, $email, $password, $repeatpassword, $params = Array(), $captcha = NULL, $sendmail = NULL)
    {
        $return['error'] = true;
        $block_status = $this->isBlocked();
        ...
        $addUser = $this->addUser($username, $email, $password, $params, $sendmail);
    ...
    */
    ...
    protected function addUser($username, $email, $password, $params = array(), &$sendmail)
    {
        $return['error'] = true;
        ...
        $query = $this->dbh->prepare("UPDATE {$this->config->table_users} SET username = ?, email = ?, password = ?, isactive = ? {$setParams} WHERE id = ?");

        $bindParams = array_values(array_merge(array($username, $email, $password, $isactive), $params, array($uid)));
    ...
    */
    ...
    {
        $query = $this->dbh->prepare("SELECT username, email, password, isactive FROM {$this->config->table_users} WHERE id = ?");
        $query->execute(array($uid));
    ...
    * @param int $uid
    * @return array $data about user, but without password
    */
    ...
    protected function addRequest($uid, $email, $type, &$sendmail)
    {
        ...
        if ($sendmail === true) {
            ...
            if ($type == "activation") {
                ...
                $mail->Subject = sprintf($this->lang['email_reset_subject'], $this->config->site_name, $key);
    ...
    */
	...
	/**
	 * Sanitize that an name is valid
	 * @param string $username
	 * @return array $return
	 */
	...
	protected function validateUsername($username) {
		...
		if (!filter_var($username, FILTER_SANITIZE_STRING)) {
			$return['message'] = $this->lang["name_invalid"];
    ...
    */
	 ...
	 * @return array $return
	 */
	public function setNewPassword($uid, $newpass, $repeatnewpass, $captcha = NULL)
	{
		$return['error'] = true;
		$block_status = $this->isBlocked();
	...
	public function changeRole($uid, $role, $password, $captcha = NULL)
	{
		$return['error'] = true;
		$block_status = $this->isBlocked();
		...
		if ($role == $user['role']) {
			$this->addAttempt();
			$return['message'] = $this->lang["newrole_match"];

			return $return;
		}

		$query = $this->dbh->prepare("UPDATE {$this->config->table_users} SET role = ? WHERE id = ?");
		$query->execute(array($role, $uid));
		...
		$return['error'] = false;
		$return['message'] = $this->lang["role_changed"];
	...
	public function changeUsername($uid, $username, $password, $captcha = NULL)
	{
		$return['error'] = true;
		$block_status = $this->isBlocked();
		...
		if ($username == $user['username']) {
			$this->addAttempt();
			$return['message'] = $this->lang["newusername_match"];

			return $return;
		}

		$query = $this->dbh->prepare("UPDATE {$this->config->table_users} SET username = ? WHERE id = ?");
		$query->execute(array($username, $uid));
		...
		$return['error'] = false;
		$return['message'] = $this->lang["username_changed"];
